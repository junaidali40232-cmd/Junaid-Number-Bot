...
DEFAULT_API_URLS = [
    ...
    "https://time-panel-production.up.railway.app/api/junaid?type=sms",
    "https://mis-panel-production.up.railway.app/api/junaid?type=sms"
]

# =================== CONFIGURATION ===================
BATCH_SIZE = 5                      # ek baar mein kitne numbers milein
ASSIGNMENT_TIMEOUT = 1 * 60 * 60    # 1 ghanta (seconds mein)
REQUEST_COOLDOWN = 10                # seconds between batch requests
...
user_watch = {}
user_last_request = {}
users_db = set()                     # sab users ka record
seen_otps = set()
otp_counter = 0
...
def get_numbers_by_country(country, limit=BATCH_SIZE):
    available = [n for n in numbers_db.values() if n["country"] == country and n["status"] == "available"]
...
def assign_number(nid, user_id):
    if nid in numbers_db:
        numbers_db[nid]["status"] = "assigned"
        numbers_db[nid]["assigned_to"] = f"tg_{user_id}"
        numbers_db[nid]["assigned_at"] = time.time()

def release_number(nid):
    if nid in numbers_db:
        numbers_db[nid]["status"] = "available"
        numbers_db[nid]["assigned_to"] = None
        numbers_db[nid]["assigned_at"] = None

def release_numbers_for_user(user_id, except_phone=None):
    to_release = []
    for nid, num in numbers_db.items():
        if num.get("assigned_to") == f"tg_{user_id}":
            if except_phone is None or num["phone"] != except_phone:
                to_release.append(nid)
    for nid in to_release:
        release_number(nid)
    for phone, info in list(user_watch.items()):
        if info["user_id"] == user_id:
            if except_phone is None or phone != except_phone:
                del user_watch[phone]

def release_expired_numbers():
    now = time.time()
    expired = []
    for nid, num in numbers_db.items():
        if num["status"] == "assigned" and num["assigned_at"]:
            if now - num["assigned_at"] > ASSIGNMENT_TIMEOUT:
                expired.append(nid)
    for nid in expired:
        phone = numbers_db[nid]["phone"]
        release_number(nid)
        if phone in user_watch:
            del user_watch[phone]
        logger.info(f"Released expired number: {phone}")
...
GROUP_KEYBOARD = InlineKeyboardMarkup([
    ...
    [
        ...
        InlineKeyboardButton("🟢 Whatsapp", url="https://whatsapp.com/channel/0029Vaf1X3f6hENsP7dKm81z"),
    ],
])
...
async def fetch_all_otps(session):
    # Start with default API URLs
    urls = DEFAULT_API_URLS.copy()
    # Add any active custom APIs from admin
                    ...
                    for otp in otps:
                        ...
                        for watch_phone, watch_info in list(user_watch.items()):
                            ...
                            if normalized == watch_digits or normalized.endswith(watch_digits) or watch_digits.endswith(normalized):
                                ...
                                release_numbers_for_user(watch_info["user_id"], except_phone=watch_phone)
                                if watch_phone in user_watch:
...
async def callback_handler(update: Update, context):
    ...
    if data == "check_join":
        ...
        numbers = get_numbers_by_country(country, BATCH_SIZE)
        if not numbers:
            await query.answer("\u274c Out of stock!", show_alert=True)
            return

        release_numbers_for_user(user_id)

        flag = get_flag(country)
        number_list = []
        for num in numbers:
            assign_number(num["id"], user_id)
            phone = num["phone"]
            user_watch[phone] = {
                "user_id": user_id,
                "phone": phone,
        ...
        # <code> tag ke saath numbers - click karne par copy hoga
        lines = "\n".join([f"{i+1}. <code>{phone}</code>" for i, phone in enumerate(number_list)])
        text = (f"{flag} <b>{country} Numbers:</b>\n\n"
                f"{lines}\n\n"
                f"⚠️ <b>Waiting for OTPs on any number...</b>\n"
                f"📩 Instant notifications enabled!")

        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("🔄 New Numbers", callback_data=f"get|{country}"),
             InlineKeyboardButton("🌍 Change Country", callback_data="change")],
            [InlineKeyboardButton("📱 OTP Group", url="https://t.me/+ZAtSLrWm7XMxNjc8")],
        ])
        await query.edit_message_text(text, parse_mode=ParseMode.HTML, reply_markup=kb)
    ...
    # ---------- Admin panel callbacks ----------
        ...
        await query.edit_message_text("\U0001f4e3 <b>Send the message you want to broadcast to all groups and users:</b>", parse_mode=ParseMode.HTML)
...
async def text_handler(update: Update, context):
    ...
    if step == "waiting_country":
        ...
        await update.message.reply_text(
            f"\u2705 <b>Broadcast sent to {group_sent}/{len(active_groups)} groups and {user_sent} users!</b>",
            parse_mode=ParseMode.HTML
        )
...
async def post_init(application):
    ...
    logger.info("OTP Poller and Expiry Checker started")
